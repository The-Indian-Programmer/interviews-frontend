...
const VerticalNavMenuGroup = ({
  ...
  const openClassCondition = id => {
    if ((menudivd && menuHover) || menudivd === false) {
      if (groupActive.includes(id) || groupOpen.includes(id)) {
        return true
      }
    } else if (groupActive.includes(id) && menudivd && menuHover === false) {
      return false
    } else {
      return null
    }
  ...
  return (
    <li
      className={classnames('nav-item has-sub', {
        ...
        'menu-divd-open': groupActive.includes(item.id),
        'sidebar-group-active':
          groupActive.includes(item.id) || groupOpen.includes(item.id) || currentActiveGroup.includes(item.id)
      })}
    >
      <Link className='d-flex align-items-center' to='/' onClick={e => ondivClick(e, item)}>
        {item.icon}
      ...
      <ul className='menu-content'>
        <div isOpen={(groupActive && groupActive.includes(item.id)) || (groupOpen && groupOpen.includes(item.id))}>
